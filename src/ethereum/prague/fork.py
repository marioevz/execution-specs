...
VALIDATOR_EXIT_PRECOMPILE_ADDRESS = hex_to_address(
    "0x229F8EdAF3c9C852E29034AF9EA3ce16b50AE017"
)
EXIT_MESSAGE_QUEUE_HEAD_STORAGE_SLOT = 2
EXIT_MESSAGE_QUEUE_TAIL_STORAGE_SLOT = 3
EXIT_MESSAGE_QUEUE_STORAGE_OFFSET = 4
MAX_EXITS_PER_BLOCK = 16
...
def apply_body(
    ...
    excess_blob_gas: U64,
    exits: Tuple[ValidatorExit, ...],
) -> Tuple[Uint, Root, Root, Bloom, State, Root, Uint, Root]:
    """
    Executes a block.
    ...
    exits :
        Validator exits contained in the block body.
    ...
    exits_trie: Trie[Bytes, Optional[Union[Bytes, ValidatorExit]]] = Trie(
        secured=False, default=None
    )
    ...
    expected_exits = calculate_exits(state)
    ensure(expected_exits == exits, InvalidBlock)

    for exit in expected_exits:
        trie_set(exits_trie, rlp.encode(exit))
    
    validator_exit_contract_code = get_account(
        state, VALIDATOR_EXIT_PRECOMPILE_ADDRESS
    ).code

    exits_system_tx_message = Message(
        caller=SYSTEM_ADDRESS,
        target=VALIDATOR_EXIT_PRECOMPILE_ADDRESS,
        gas=SYSTEM_TRANSACTION_GAS,
        value=U256(0),
        data=b"",
        code=validator_exit_contract_code,
        depth=Uint(0),
        current_target=VALIDATOR_EXIT_PRECOMPILE_ADDRESS,
        code_address=VALIDATOR_EXIT_PRECOMPILE_ADDRESS,
        should_transfer_value=False,
        is_static=False,
        accessed_addresses=set(),
        accessed_storage_keys=set(),
    ...
    exits_system_tx_env = vm.Environment(
        caller=SYSTEM_ADDRESS,
        origin=SYSTEM_ADDRESS,
        block_hashes=block_hashes,
        coinbase=coinbase,
    ...
    process_message_call(exits_system_tx_message, exits_system_tx_env)
...
def calculate_exits(
    ...
    """
    Calculates the expected validator exits list after all transactions have been applied.

    Parameters
    ----------
    state :
        State after all block's transactions have been applied.

    Returns
    -------
    expected_exits : `tuple`
        Tuple of expected exits.
    """
    queue_head_index = get_storage(state, VALIDATOR_EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_HEAD_STORAGE_SLOT)
    queue_tail_index = get_storage(state, VALIDATOR_EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_TAIL_STORAGE_SLOT)
    ...
    for i in range(num_exits_to_dequeue):
        ...
        source_address = Address(get_storage(state, VALIDATOR_EXIT_PRECOMPILE_ADDRESS, queue_storage_slot).to_be_bytes32()[0:20])
        validator_pubkey_1 = get_storage(state,VALIDATOR_EXIT_PRECOMPILE_ADDRESS, queue_storage_slot + 1).to_be_bytes32()
        validator_pubkey_2 = get_storage(state,VALIDATOR_EXIT_PRECOMPILE_ADDRESS, queue_storage_slot + 2).to_be_bytes32()
        validator_pubkey = (
            validator_pubkey_1[0:32] + validator_pubkey_2[0:16]
        )
        exit = ValidatorExit(
